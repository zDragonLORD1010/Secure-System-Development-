...
Список это **шкафчик**, в который мы помещаем какие-то **переменные**. Например, если я хочу положить 3 яблока (🍎🍎🍎) в шкаф на разные полки, то на каждой полке у меня будет лежать по яблоку (🍎). Получается в шкафчике у меня будет `[🍎, 🍎, 🍎]`.

С массивами мы можем работать также как и со строками, только в строках при обращении к символу через `string[i]` мы получим только один символ, а в массиве при обращении `array[i]` мы можем получить, как один символ, так и строку любой длинны.

## Ввод и вывод списков (массивов)
...
# Двумерные списки (массивы) в Python
